...
INSERT INTO lokale (
    ...
    kod_pocztowy,
    powierzchnia_w_m2,
    miejsca_dla_klientow
) VALUES (
    id_lokale_seq.NEXTVAL,
    'waleczna 2',
    'Zamkowo',
    '12-345',
    50,
...
INSERT INTO dostawcy_skladnikow (
    skladnik,
    dostawca,
    cena
) VALUES (
...
exec wstaw_danie('pizza', 20.99, skladniki_lista(1));

exec wstaw_danie('sernik', 4.99, skladniki_lista(1));

exec wstaw_zamowienie_na_dowoz('Rycerska 22', 'Wioska', 1, 1, dania_lista(1, 2), ilosci_lista(3, 7));

exec wstaw_zamowienie_na_dowoz('Kanclerska 2', 'Miasto', 1, 1, dania_lista(1), ilosci_lista(4));

select id,adres,miasto,najblizszy_lokal,dostawca from zamowienia_na_dowoz;

select zamowienie,danie,ilosc from zamowione_jedzenie;

delete from zamowienia_na_dowoz where miasto='Miasto';

select id,adres,miasto,najblizszy_lokal,dostawca from zamowienia_na_dowoz;

select zamowienie,danie,ilosc from zamowione_jedzenie;

select id,imie,nazwisko,lokal,stanowisko,data_zatrudnienia from pracownicy where imie='Tomasz' and nazwisko = 'Kowalski';
